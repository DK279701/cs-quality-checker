...
def collect_data(token, inbox_ids, fetch_prog):
    ...
    for idx, inbox in enumerate(inbox_ids, start=1):
        ...
        while True:
            ...
            if err:
                st.error(f"Błąd pobierania konw {inbox}: {err}")
                break
        ...
        # pobierz wiadomości każdej konwersacji
        for conv in convs:
            cid = conv.get("id")
            url_msgs = f"https://api2.frontapp.com/conversations/{cid}/messages"
            js2, err2 = safe_get(url_msgs, headers)
            if err2:
...
if st.button("▶️ Pobierz i analizuj"):
    ...
    # --- Filtrowanie po zakresie dat (lokalnie) ---
    df["Created_date"] = df["Created At"].dt.date
    ...
    SYSTEM = (
        "Jesteś Menedżerem CS w Bookinghost. Oceń jakość wiadomości w skali 1–5, "
        "uwzględniając empatię, poprawność, zgodność z procedurami i ton.\n"
        "Odpowiedz formatem:\nOcena: X/5\nUzasadnienie: • pkt1\n• pkt2"
    )
